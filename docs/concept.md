...
- **Spring MVC, y no WebFlux.** El modelo reactivo de WebFlux solo rinde si toda la cadena lo es, y aquí el acceso a datos se hace con JPA sobre MySQL, que es bloqueante: mezclarlos daría lo peor de ambos mundos. Además, WebFlux está pensado para miles de conexiones concurrentes, un escenario muy lejano al de este proyecto, y a cambio complica la depuración y obliga a conocer programación reactiva para tocar cualquier cosa. Por último, con Java 21 los hilos virtuales (`spring.threads.virtual.enabled`) aportan buena parte de esa capacidad de concurrencia sin cambiar el modelo de programación.
